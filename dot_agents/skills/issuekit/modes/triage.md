...
**Title repairs are a previewed batch.** A title edit rewrites what every list, notification, and PR link shows, so it is an outward-facing mutation and the [label exemption](../SKILL.md#preflight-every-mode) does not cover it. Propose all renames as one `old -> new` table, get one OK for the whole batch, then apply each with `gh issue edit <n> --title "<new>"`. A row the user rewrites or strikes is applied as corrected or skipped.

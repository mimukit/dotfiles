...
3. **No persistence.** State lives in memory. Persistence is usually the thing being questioned, not something to lean on. When the question genuinely involves a database, use a scratch one named so nobody mistakes it, such as `PROTOTYPE_wipe_me`.

...
- **Private repos.** Inline rendering needs a public repo. On a private one, `verify-assets.sh check` fails, so skip publishing and hand off the local bundle path for manual attachment rather than embedding dead links.
- **Zero clone bloat is the point.** Proof is never committed to the branch; it lives on `refs/verify-assets/*`, which default clones skip. Old refs accumulate on the remote but never in anyone's clone, so prune them with `verify-assets.sh delete <slug>` once a PR merges.

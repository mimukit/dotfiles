...
**Next.** The stamp is the whole point of finishing a grill, so say what it unlocks: a hardened plan is ready to become issues, and **issuekit** reads that stamp to file them as `ready` (safe to work unattended) rather than `needs-planning`. Without a plan file, name what the decisions feed instead, either the build itself (**implementkit**) or a decision record (**domainkit**). Don't start either; grilling ends at the shared understanding.

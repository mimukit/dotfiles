...
name: refactorkit
description: >-
  Survey an existing codebase for the structural change worth making, whether shallow interfaces, adapter sprawl, poor locality, or untested coupling, then rank the candidates, crown one, and write it up as a reviewable proposal. Use when the user says "where should I refactor", "what's wrong with this codebase's structure", "find refactoring opportunities", "this code is hard to change", "improve our architecture", "audit the module boundaries", or "/refactorkit". It proposes and never edits code.
license: MIT
...
| `weak` | gates pass, but the win is stylistic, and nothing changes about what a caller must know | listed, **never crowned** |
...
- a repo-root context or glossary file (commonly `CONTEXT.md`) for the ubiquitous language, which is where the project's own domain terms come from, and where its naming conventions usually surface;
...
The terminal line has to carry the coverage, as in *"scanned 38 files across `src/`; nothing cleared the deletion test"*, or an empty result is indistinguishable from a scan that never ran.
...
- **Follow the repo over these defaults.** An established artifact location, a documented convention, or a stated policy in the repo's agent-guide file (`CLAUDE.md` or an equivalent) wins, so say that you followed it.
